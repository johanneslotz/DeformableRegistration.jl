include("../src/helpers/matrixFreeHelpers.jl")


function ssdDistanceNoHessian(referenceImage::regImage,templateImage::regImage,
  ...
  if(doDerivative)
      if(matrixFree)
          dFunctionValue,d2FunctionValue = ssdHDerivativesMatrixFree(dX_transformedImage,dY_transformedImage,residual,centeredGrid,prodH,N,parametricOnly,doDerivative,doHessian)
      else
          dFunctionValue,d2FunctionValue = ssdHDerivativesMatrixBased(dX_transformedImage,dY_transformedImage,residual,centeredGrid,prodH,N,parametricOnly,doDerivative,doHessian)
      end
  else
...
#
#   if(doHessian)
#     d2FunctionValue = 0.0 #pixelSize .* 2*speye(N) #identity or zero
#   else
#     d2FunctionValue = 0.0
#   end
#
...
function ssdHDerivativesMatrixFree(dX_transformedImage::Array{Float64,1},
                                  dY_transformedImage::Array{Float64,1},
                                  residual::Array{Float64,1},
                                  centeredGrid::Array{Float64,1},
                                  pixelSize::Number,N::Number,
  ...
  if(doHessian)
     if(parametricOnly)
       ...
       d2FunctionValue(x) =  0.0 #0.001 * pixelSize .* x # identity or zero
     end
  else
     d2FunctionValue = 0.0
  end
