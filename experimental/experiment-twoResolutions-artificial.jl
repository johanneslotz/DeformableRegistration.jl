...
options.regularizerWeight = 1
        ...
        plot ? (
            ...
            visualizeResults(refImgFine, plotTemImg,
                ...
                suptitle="pre registration", filename = "artificial-example-DDS-preRegistration.png");) : 0
...
displacements = Array{scaledArray}(2)
    @time for i = 1:2
        ...
        plot ? (figure();  visualizeResults(refImgFine, plotTemImg, displacement= tempDisplacement, showDeformationImage=showDeformationImage, suptitle="patch $i only", filename = "artificial-example-DDS-patch-$i.png");) : 0
    end
...
displacements = Array{scaledArray}(2)
    @time for i = 1:2
        displacements[i] = registerInTwoResolutions(refPatches[i], temPatches[i],
                    ...
                    gradientDescentOnly=false) # regularizerOperator=createDiffusiveOperatorCentered
        tempDisplacement = interpolateDeformationField(displacements[i], getCellCenteredGrid(refImgFine))
        #plot ? (figure();  visualizeResults(refImgFine, plotTemImg, displacement= tempDisplacement, showDeformationImage=showDeformationImage, suptitle="fixed grid resolution at level", filename = "artificial-example-DDS-patch-$i.png");) : 0
    end
    ...
    plot ? (figure();  visualizeResults(refImgFine, plotTemImg, displacement= combinedDisplacement, showDeformationImage=showDeformationImage, suptitle="combined patches", filename = "artificial-example-DDS-patches-combined.png");) : 0
